...
- API data can be unreliable and limited.
    - Sometimes API's works and sometimes it doesn't. The uptime is not always detailed on the website for the API, especially when it's free, so one would need to monitor it over a period of time and retieving various volumes of information before knowing if it could be used for their project. 
